...
contains
  ...
  SUBROUTINE openmp_grid_init()
     !$ use omp_lib
     INTEGER :: idx, idy, idz, ival, jval ! For tile extent calculation
     INTEGER :: ierr, nwidth
     INTEGER :: ji,jj, ith
...
!$   nthreads = omp_get_max_threads()
     ...
     WRITE (*,"('OpenMP thread tiling using grid of ',I3,'x',I3)") ntilez,ntiley
     ...
     DO jj = 1, ntiley, 1
        ...
        DO ji = 1, ntilez, 1
           ...
           ELSE
              tile(ith)%kstop = MIN(ival + idxtmp - 1, jpk)
           ...
           max_tile_width  = MAX(max_tile_width, (tile(ith)%kstop - tile(ith)%kstart + 1) )
           max_tile_height = MAX(max_tile_height, (tile(ith)%jstop - tile(ith)%jstart + 1) )
     ...
     WRITE(*,"(/'Mean tile size = ',F6.1,' cols = ',F7.1,' KB')") &
                                   ...
                                   REAL(8*jpk*nvects_sum)/REAL(ntiles*1024)
  ...
  subroutine zwx_compute(zwx,tmask,mydomain)
    ...
    !
    !$omp parallel do default(none), private(ji, jj, jk), &
    !$omp shared(jpi, jpj, jpk, zwx, mydomain, tmask)
    DO jk = 2, jpk-1
       DO jj = 1, jpj
          DO ji = 1, jpi
  ...
  subroutine zslpx_compute(zslpx,zwx)
    ...
    !
    !$omp parallel do default(none), private(ji, jj, jk), &
    !$omp shared(jpi, jpj, jpk, zwx, zslpx)
    DO jk = 2, jpk-1    
       DO jj = 1, jpj
          DO ji = 1, jpi
  ...
  subroutine zwx2_compute(zwx,pwn,mydomain,zind,zslpx)
    ...
    !$omp private(ji, jj, jk, z0w, zalpha, zw, zzwx, zzwy), &
    !$omp shared(zdt, zbtr, jpi, jpj, jpk, zwx, zslpx, zind, mydomain, pwn)
    DO jk = 2, jpk
       DO jj = 2, jpj-1     
          DO ji = 2, jpi-1
  ...
  subroutine mydomain_compute(mydomain,zwx)
    ...
    zbtr = 1.
    !$omp parallel do default(none), &
    !$omp private(ji, jj, jk, ztra), shared(jpi, jpj, jpk, zbtr, zwx, mydomain)
    DO jk = 1, jpk-1
       DO jj = 2, jpj-1     
          DO ji = 2, jpi-1
